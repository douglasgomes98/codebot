...
import { IPathResolver, ProjectContext, ErrorType } from '../types';
...
export class PathResolver implements IPathResolver {
  ...
  resolveTemplatePath(projectContext: ProjectContext, templateName: string): string {
    ...
    if (projectContext.isMultiProject) {
      const projectTemplatePath = path.join(projectContext.projectRoot, 'templates', sanitizedTemplateName);
      return this.normalizePath(projectTemplatePath);
    }

    // For single project or fallback, use workspace-level templates
    const workspaceTemplatePath = path.join(projectContext.workspaceRoot, 'templates', sanitizedTemplateName);
  ...
  resolveConfigPath(projectContext: ProjectContext): string {
    ...
    if (projectContext.isMultiProject) {
      const projectConfigPath = path.join(projectContext.projectRoot, 'codebot.config.json');
      return this.normalizePath(projectConfigPath);
    }

    // For single project, use workspace-level config
    const workspaceConfigPath = path.join(projectContext.workspaceRoot, 'codebot.config.json');
   ...
   */
  resolveRelativePath(projectContext: ProjectContext, relativePath: string): string {
    // Check for dangerous security patterns first
   ...
   */
  private resolveBasePath(projectContext: ProjectContext, selectedFolderPath?: string): string {
   ...
   */
  private isWithinProjectBoundaries(targetPath: string, projectContext: ProjectContext): boolean {
    const normalizedTarget = this.normalizePath(targetPath);
    const normalizedProjectRoot = this.normalizePath(projectContext.projectRoot);
    const normalizedWorkspaceRoot = this.normalizePath(projectContext.workspaceRoot);

    // Path must be within either project root or workspace root
    return normalizedTarget.startsWith(normalizedProjectRoot) || 
  ...
  /**
   * Synchronously checks if a path exists (used for template discovery)
   */
